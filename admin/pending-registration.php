...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    if (isset($_POST['acceptUser'])) {
        ...
        $user->updateStatus($userId, 1); // Status indicating acceptance
    } elseif (isset($_POST['rejectUser'])) {
        $userId = $_POST['rejectUser'];
        $user->getByFilters($userId);
        include_once "../class-file/EmailSender.php";
        $emailSender = new EmailSender();
        $emailSender->sendMail($user->email, "Account Rejection", "Dear User, Your account has been rejected. Please contact support for more information. Thank you.");
        $user->deleteByUserId($userId);
    }
}

// Fetch all pending users
$allPendingUsers = $user->getByFilters(null, 0, null, "client");
// echo "<pre>";
// print_r($allPendingUsers);
// echo "</pre>";
// exit();
?>
    ...
    <div class="site-wrap">
        ...
        <div class="container py-4">
            ...
            <div class="row" id="cardContainer">
                <?php foreach ($allPendingUsers as $u) { ?>
                    <div class="col-md-6">
                        <div class="card p-3">
                                ...
                                <form action="" method="post">
                                    ...
                                    <script>
                                        document.querySelector('.btn-reject').addEventListener('click', function() {
                                            alert('Sending email to <?php echo $user->email; ?> for account registration rejection.');
                                        });
                                    </script>
                                </form>
                            </div>
